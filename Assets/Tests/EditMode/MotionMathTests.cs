...
namespace SpiderVer2.Tests
{
    ...
    {
        const float Soft = 82f, Max = 112f, Dt = 1f / 120f;

        [Test]
        public void SOFT_SPEED_아래는_건드리지_않는다()
        {
            var v = new Vector3(30f, -20f, 10f);
            Assert.AreEqual(v, MotionMath.ClampSpeed(v, Soft, Max, Dt));
        }
        ...
        public void MAX_SPEED를_절대_넘기지_않는다()
        {
            ...
            var outv = MotionMath.ClampSpeed(v, Soft, Max, Dt);
        ...
        {
            var outv = MotionMath.ClampSpeed(Vector3.zero, Soft, Max, Dt);
